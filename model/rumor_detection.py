...
class RumorDetection(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim):
        ...
        self.norm = torch.nn.BatchNorm1d(14 + 10*3 + 1 + 1 + 1)
        self.linear = torch.nn.Linear(1 + 18, 1)
        self.sigmoid = torch.nn.Sigmoid()

        self.final_predict = nn.Sequential(
            nn.Linear(14 + 10*3 + 1 + 1 + 1 + 1, 10),
            nn.ReLU(),
    ...
    def forward(self, text_embedding, verified, messages, followers,
                ...
                total_at_num, reposts_tree_max_length, useful_comment_rate,word_rumor_score_base,positive_max,negative_max,neutral_max,positive_original,negative_original,neutral_original,sentence_similarity
                ):
        ...
        norm_input = torch.cat([messages, followers, friends, comments, likes, reposts,
                                ...
                                emb,
                                comment_embedding,
                                location_embedding,word_rumor_score_base,sentence_similarity
                                ], dim=1)
        # norm_input = torch.cat([messages, followers, friends, comments, likes, reposts,
        #                         positive, negative, neutral, tree_avg_length, friends_followers_rate, total_at_num,
        #                         reposts_tree_max_length, useful_comment_rate,
        #                         text_embedding,
        #                         location_embedding, word_rumor_score_base
        #                         ], dim=1)
