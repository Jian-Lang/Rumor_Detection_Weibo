...
def test_ens(args, model_list, logger):
    ...
    test_data_loader = DataLoader(dataset=test_data, batch_size=len(pd.read_pickle(args.dataset_path)), collate_fn=custom_collate_fn)
        ...
        for batch in tqdm(test_data_loader, desc='Testing'):
            ...
            for i in range(len(model_list)):
                ...
                output = model(text_embedding, verified, description, gender, messages, followers,
                               location_embedding, friends, verified_type, comments, pics, likes, reposts,
                               time_interval,
                               has_url, emb, comment_embedding, positive_avg,negative_avg,neutral_avg,reposts_tree_avg_length,is_positive,is_negative,
                               friends_followers_rate, comments_reposts_rate, likes_reposts_rate,total_at_num,reposts_tree_max_length,useful_comment_rate,positive_max, negative_max, neutral_max, positive_original, negative_original, neutral_original,word_rumor_score_base)
                if i == 0:
                    total_output = output
                else:
                    total_output += output
...
def main():
    ...
    parser.add_argument('--dataset_path', default=r'D:\RumorDetection\data\test_data\test_with_user.pkl', type=str, help='path of dataset folder')
    ...
    folder_path = r"D:\RumorDetection\train_results\train_mlp_rumor_train_BCE_2023-12-08_12-16-01\trained_model"
    ...
    df['output_20'] = output.tolist()

    df.to_pickle(r'ens14.pkl')
